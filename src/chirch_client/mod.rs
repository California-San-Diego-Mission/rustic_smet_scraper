...
use crate::response_handling::{unwrap_response_body, response_status_is_ok};
...
pub async fn login_to_ref_manager(username: &str) {
    ...
    let request_builder = client.get("https://referralmanager.churchofjesuschrist.org")
    ...
    let output = match request_result.await {
    ...
    let church_site_body = output.await.unwrap();
    ...
    let mut response_body = unwrap_response_body(response).await;
    ...
    response = client
        ...
        .await;  // Wait for the request to complete and handle any errors
    if response_status_is_ok(&response) == false {
        panic!("ChurchInvalidCreds");
    }
    response_body = unwrap_response_body(response).await;
